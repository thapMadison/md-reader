...
// Amber Parchment: a warm, amber-accented light theme — Phycat's light half
// applied as a single hue over an otherwise fixed design. Adapted from Phycat's
// light base plus its own "caramel" variant (`phycat.light.css` +
// `phycat.caramel.css` in the Phycat Typora theme, upstream stylesheets not
...
// single hue applied to an otherwise fixed design, and Phycat's own "caramel" is
// the amber one.
//
// That fixed part is shared verbatim with orchid-vellum; only the ramp, the
// chrome and the feedback tones differ between the two files.
export const tokens: ThemeTokens = {
  // No light variant defines --bg-color at all, so `#write` keeps Typora's white.
  '--bg': '#ffffff',
  // --element-color-soo-shallow, the palest step of the ramp. The light base
  // never fills the sidebar either — `#typora-sidebar { background-color:
  // var(--bg-color) }` against a variable nothing defines — so the chrome has to
  // come from the ramp, and this is the step that reads as a panel rather than a
  // block of color.
  '--chrome': '#fffbeb',
  '--fg': '#333333',
  // Phycat's light base authors no muted tone — it has body text, quote text and
  // the accent ramp and nothing between. Mixed from --element-color-linecode
  // toward the page until it is clearly quieter than #333 without dropping below
  // AA on either the page (5.18) or the chrome (4.99).
  '--muted': '#7d6a52',
  ...
  '--h5-fg': '#333333',
  '--h6-fg': '#333333',
  // Warm hairline. The base's own interior rule is a flat #f0f0f0; tinted into
  // the ramp's hue so borders belong to the theme rather than sitting neutral
  // inside a strongly warm page.
  '--border': '#f0e7d5',
  // `.cm-s-inner.CodeMirror { background: #f8f8f8 }` — hardcoded in the base and
  // therefore the same neutral gray in every light variant, Caramel included.
  '--code-bg': '#f8f8f8',
  // `blockquote { background-color: var(--element-color-soo-shallow) }`, with no
  // border at all upstream — the wash is the whole treatment.
  '--quote-bg': '#fffbeb',
  '--hl': 'rgba(245,158,11,0.14)',
  // Phycat's quotes have no accent bar; this app's do. --element-color darkened
  // along its own hue from 2.15 to 3.12 on the page, the ratio a solid graphic
  // element needs to register as one.
  '--quote-accent': '#cb8208',
  ...
  '--heading-accent': '#f59e0b',
  '--heading-accent-soft': 'rgba(245,158,11,0.35)',
  '--h2-accent': '#f59e0b',
  '--h2-accent-soft': 'rgba(245,158,11,0.35)',
  '--h3-accent': '#f59e0b',
  '--h3-accent-soft': 'rgba(245,158,11,0.35)',
  '--h4-accent': '#f59e0b',
  '--h4-accent-soft': 'rgba(245,158,11,0.35)',
  '--h5-accent': '#f59e0b',
  '--h5-accent-soft': 'rgba(245,158,11,0.35)',
  '--h6-accent': '#f59e0b',
  '--h6-accent-soft': 'rgba(245,158,11,0.35)',
  ...
  // var(--element-color-deep); font-weight: 700 }`.
  '--table-header-bg': '#fffbeb',
  '--table-header-fg': '#b45309',
  ...
  '--table-row-alt': 'rgba(245,158,11,0.03)',
  ...
  '--table-row-hover': 'rgba(245,158,11,0.06)',
  ...
  // there is no second or third palette color to borrow, and the ramp's own
  // amber is already the link, so a warn slot taken from it would collide.
  // Chosen instead as the quietest red/amber/green that clear AA on both the
  // page and the chrome, kept dark enough to sit with the warm serif page.
  '--danger': '#c62828',
  ...
  '--chrome-fg': '#78350f',
  '--chrome-muted': '#7d6a52',
  '--chrome-border': '#f0e7d5',
  // --glass-bg-color is `#f59e0b15` — --element-color at 8% alpha.
  '--chrome-hl': 'rgba(245,158,11,0.14)',
  ...
  // variant renders code identically, regardless of its ramp. It was cut against
  // a white editor and struck here against the base's own #f8f8f8 fence, where
  // most of it landed short; seven of the twelve are darkened along their own
  // hue below. Hue and relative rank are preserved throughout, so the block
