...
// Orchid Vellum: a cool, orchid-accented light theme — Phycat's light half
// applied as a single hue over the same fixed design as amber-parchment.
// Adapted from Phycat's light base plus its own "mauve" variant
// (`phycat.light.css` + `phycat.mauve.css` in the Phycat Typora theme, upstream
// stylesheets not present in this repo) — kept here as a lineage note rather
// than this theme's identity, so the comments below still quote the
...
// single hue applied to an otherwise fixed design, and Phycat's own "mauve" is
// the orchid one.
//
// That fixed part is shared verbatim with amber-parchment; only the ramp, the
...
export const tokens: ThemeTokens = {
  ...
  // for both light variants, so this trio matches amber-parchment's exactly.
  '--danger': '#c62828',
  ...
  // the same one amber-parchment carries. It was cut against a white editor and
